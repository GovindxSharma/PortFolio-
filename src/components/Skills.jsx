...
export default function Skills() {
  ...
  return (
    <section
      id="skills"
      className="relative py-2 sm:py-3 px-1 sm:px-4 overflow-hidden"
    >
      {/* Background Glows */}
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="text-center mb-2.5 sm:mb-3">
          <div className="inline-flex items-center gap-2 rounded-full border border-cyan-500/30 bg-white/70 dark:bg-[#12141c] px-3 py-0.5 text-xs font-mono text-cyan-600 dark:text-cyan-400 font-bold mb-1.5">
            <Zap size={12} className="text-cyan-400" />
            <span>[ AWAKENED SKILL MATRIX // PRACTICAL ENGINEERING APPLIED ]</span>
          </div>

          <h2 className="text-xl sm:text-3xl md:text-4xl font-black text-slate-900 dark:text-[#f8fafc] font-['Rajdhani',sans-serif] tracking-tight leading-tight">
            Technical <span className="text-cyan-600 dark:text-cyan-400">Mastery</span> & Production Spells
          </h2>

          <p className="mt-0.5 text-xs text-slate-600 dark:text-slate-400 font-mono max-w-2xl mx-auto">
        ...
        {activeHighlight && (
          <motion.div
            ...
            className="mb-2.5 p-2.5 sm:p-3 rounded-2xl border border-cyan-400/50 bg-gradient-to-r from-cyan-500/10 via-white dark:via-[#101218] to-violet-600/10 shadow-[0_2px_15px_rgba(6,182,212,0.12)] backdrop-blur-xl"
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-slate-200 dark:border-white/10 pb-1.5 mb-1.5">
              <div className="flex items-center gap-2">
                <span className="flex h-2 w-2 rounded-full bg-cyan-400 animate-ping" />
                <span className="text-xs font-mono font-bold uppercase text-cyan-600 dark:text-cyan-400 tracking-wider">
                  SPOTLIGHT: {activeHighlight.skill}
                </span>
                <span className="text-[9.5px] font-mono px-2 py-0.2 rounded-full bg-cyan-500/20 text-cyan-700 dark:text-cyan-300 font-black border border-cyan-500/30">
                  {activeHighlight.metric}
                </span>
              </div>
              <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400 hidden sm:inline">
                Click any skill badge below to inspect application & impact
              </span>
            </div>

            <div className="grid sm:grid-cols-12 gap-2 text-xs font-mono">
              <div className="sm:col-span-4">
                <span className="text-[9px] uppercase text-slate-500 dark:text-slate-400 font-bold block">
                  Production Application:
                </span>
                <p className="font-bold text-slate-900 dark:text-white font-['Rajdhani',sans-serif] text-xs sm:text-sm truncate">
              ...
              <div className="sm:col-span-8">
                ...
                <p className="text-slate-700 dark:text-slate-300 font-sans leading-relaxed text-[11px] sm:text-xs">
          ...
          {filterCategories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-[11px] font-mono font-bold whitespace-nowrap transition-all duration-200 ${
                  isSelected
        ...
        {/* SKILLS GRID (4 COLUMNS ON DESKTOP TO FIT 2 CLEAN ROWS) */}
          ...
          {filteredGroups.map((group, i) => {
            ...
            return (
              <motion.div
                ...
                onMouseEnter={() => soundFX.playHover()}
                className="
                  group relative rounded-xl sm:rounded-2xl p-3 sm:p-3.5
                  border border-slate-200 dark:border-white/10
                  bg-white/90 dark:bg-[#101218]/90
                  backdrop-blur-xl
                  hover:border-cyan-400/50
                  hover:shadow-[0_4px_15px_rgba(6,182,212,0.12)]
              ...
              >
                <div>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-lg bg-gradient-to-br from-cyan-500/20 to-violet-600/20 border border-cyan-500/30 text-cyan-600 dark:text-cyan-400 shadow-sm">
                      <Icon className="h-4 w-4" />
                    </div>

                    <span className="text-[9px] font-mono font-bold px-2 py-0.2 rounded-full bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border border-cyan-500/20">
                      ⚡ {group.mana}
                    </span>
                  </div>

                  {/* Category Title */}
                  <span className="text-[8.5px] sm:text-[9px] font-mono tracking-wider text-cyan-600 dark:text-cyan-400 block uppercase font-bold">
                    {group.category}
                  </span>

                  <h3 className="text-sm sm:text-base font-bold font-['Rajdhani',sans-serif] text-slate-900 dark:text-white mt-0.5 mb-2 group-hover:text-cyan-500 dark:group-hover:text-cyan-400 transition-colors">
                    {group.title}
                  </h3>
                      ...
                      return (
                        <button
                          ...
                          className={`
                            ...
                            px-2 py-0.5
                            rounded-md
                            border transition-all duration-150 text-left leading-tight
                            ${
                              isHighlighted
                                ? "border-cyan-400 bg-cyan-500/15 text-cyan-700 dark:text-cyan-300 font-bold shadow-[0_0_8px_rgba(6,182,212,0.3)]"
                        ...
                        >
                          {skill}
                          {hasCaseStudy && (
                            <span className="ml-1 text-[8px] text-cyan-500 opacity-75">●</span>
                          )}
                        </button>
                      );
                    })}
                ...
                {/* Bottom Power Meter */}
                <div className="mt-2.5 pt-2 border-t border-slate-200 dark:border-white/5">
