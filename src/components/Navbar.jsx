...
export default function Navbar({
  ...
  const { darkMode, isRedGate, toggleTheme, toggleRedGate } = useTheme();
  ...
  return (
    <>
      ...
      <header className="fixed top-3 sm:top-5 left-0 right-0 z-40 px-3 pointer-events-none flex justify-center">
        <div className="relative pointer-events-auto">

          {/* Glowing Ambient Halo */}
          <div className="absolute -inset-1 rounded-full bg-gradient-to-r from-cyan-500/30 via-blue-500/20 to-violet-600/30 blur-md opacity-70" />

          {/* Sleek Floating Capsule Bar */}
          <div className="relative flex items-center gap-1.5 sm:gap-3 rounded-full border border-white/10 dark:border-white/10 bg-white/90 dark:bg-[#101218]/90 py-1.5 px-2.5 sm:px-4 backdrop-blur-2xl shadow-[0_10px_35px_rgba(0,0,0,0.4)]">
            ...
            >
              ...
              <div className="text-left hidden xs:block sm:block">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[8px] font-mono px-1 py-0.2 rounded bg-cyan-500/20 text-cyan-400 font-extrabold border border-cyan-500/40">
                    S-RANK
                  </span>
                </div>
              </div>
            ...
            >
              <span className="text-cyan-400">{navItems[currentSectionIndex]?.floor}</span>
            ...
            <button
              ...
              onMouseEnter={() => soundFX.playHover()}
              className="flex items-center gap-1 px-2.5 sm:px-3 py-1 rounded-full bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/30 text-[10px] sm:text-xs font-mono font-bold text-cyan-500 dark:text-cyan-400 transition active:scale-95"
            ...
            <button
              ...
              className={`p-1.5 sm:p-2 rounded-full border transition ${
                hudOpen
                  ...
                  : "border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-[#161922] text-slate-700 dark:text-slate-300 hover:text-cyan-400 hover:border-cyan-400"
              }`}
            ...
            {hudOpen && (
              <motion.div
                ...
                className="
                  ...
                  bg-[#101218]/95 p-4 sm:p-5
                  text-white shadow-[0_20px_50px_rgba(0,0,0,0.8)]
                  backdrop-blur-3xl z-50
                "
              >
                {/* Header Strip */}
                <div className="flex items-center justify-between border-b border-white/10 pb-2.5 mb-3 font-mono text-[10px] text-cyan-400">
                  <span className="flex items-center gap-1.5">
                    <Compass size={12} /> COMMAND DECK & REALM SELECTOR
                  </span>
                  <span>7 REALMS AVAILABLE</span>
                </div>
                ...
                <div className="grid grid-cols-4 gap-2 mb-4">
                  ...
                  <button
                    ...
                    className="flex flex-col items-center justify-center gap-1 p-2.5 rounded-2xl bg-[#141720] border border-white/5 hover:border-cyan-400/40 text-slate-300 hover:text-cyan-300 transition"
                  >
                    {darkMode ? <Sun size={16} className="text-cyan-400" /> : <Moon size={16} className="text-indigo-400" />}
                    <span className="text-[9px] font-mono">{darkMode ? "Light" : "Dark"}</span>
                  </button>
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center justify-center gap-1 p-2.5 rounded-2xl border transition ${
                      isRedGate
                        ...
                        : "bg-[#141720] border-white/5 text-slate-400 hover:text-red-400"
                    }`}
                  >
                    <Flame size={16} />
                    <span className="text-[9px] font-mono">{isRedGate ? "Active" : "Red Gate"}</span>
                  </button>
                  ...
                  <button
                    ...
                    className="flex flex-col items-center justify-center gap-1 p-2.5 rounded-2xl bg-[#141720] border border-white/5 hover:border-cyan-400/40 text-slate-300 hover:text-cyan-300 transition"
                  >
                    {isMuted ? <VolumeX size={16} className="text-slate-500" /> : <Volume2 size={16} className="text-cyan-400" />}
                    <span className="text-[9px] font-mono">{isMuted ? "Muted" : "Audio"}</span>
                  </button>
                  ...
                  <button
                    ...
                    className="flex flex-col items-center justify-center gap-1 p-2.5 rounded-2xl bg-[#141720] border border-white/5 hover:border-cyan-400/40 text-cyan-400 transition"
                  >
                    <Terminal size={16} />
      ...
      <div className="fixed bottom-6 left-1/2 z-40 hidden md:block -translate-x-1/2">
        <div className="relative group">
          <div className="absolute -inset-1 rounded-full bg-gradient-to-r from-cyan-500 via-blue-500 to-violet-600 blur-md opacity-30 group-hover:opacity-60 transition duration-500" />

          <div className="relative flex items-center gap-1.5 rounded-full border border-white/10 bg-[#101218]/90 p-2.5 backdrop-blur-3xl shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
            {navItems.map((item, idx) => {
              const Icon = item.icon;
              const isActive = currentSectionIndex === idx;
              ...
              return (
                <div key={item.id} className="relative group/item">
                  ...
                  >
                    ...
                    <Icon
                      className={`relative z-10 h-5 w-5 transition-colors ${
                        isActive
                          ? "text-white"
                          : "text-slate-400 group-hover/item:text-cyan-400"
                      }`}
                    />
                  </motion.button>
                  ...
                  <div className="pointer-events-none absolute -top-11 left-1/2 -translate-x-1/2 opacity-0 group-hover/item:opacity-100 transition-all duration-200 scale-95 group-hover/item:scale-100">
                    <div className="px-2.5 py-1 text-[11px] font-mono font-bold rounded-lg bg-[#0c0d12] text-cyan-300 border border-white/10 shadow-xl whitespace-nowrap">
