...
export default function ProjectsExperience() {
    ...
    return (
      <div
        ...
        className={`
          ...
          rounded-xl sm:rounded-2xl p-3.5 sm:p-4
          border transition-all duration-200
          bg-white/90 dark:bg-[#101218]/95 backdrop-blur-xl
          hover:-translate-y-1 cursor-pointer group
          ${
            isRankS
              ? "border-cyan-400/70 shadow-[0_0_20px_rgba(6,182,212,0.16)] hover:shadow-[0_0_25px_rgba(6,182,212,0.3)]"
      ...
      >
        <div>
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <div
                className={`flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-lg text-base ${
                  isRankS
                    ? "bg-gradient-to-br from-cyan-400 to-violet-600 text-white shadow-[0_0_12px_rgba(6,182,212,0.4)]"
            ...
            {/* Rank Badge */}
            {isRankS ? (
              <span className="inline-flex items-center gap-1 text-[10px] font-mono font-black px-2 py-0.5 rounded-full bg-cyan-500/20 text-cyan-400 border border-cyan-400/50 shadow-[0_0_8px_rgba(6,182,212,0.3)]">
                <Sparkles size={10} /> RANK S
              </span>
            ) : (
              <span className="text-[9.5px] font-mono px-2 py-0.2 rounded bg-slate-200 dark:bg-[#161922] text-slate-700 dark:text-slate-300 border border-transparent dark:border-white/5">
                RANK A
              </span>
            )}
          </div>
          ...
          {/* Project Title */}
          <h3 className="text-base sm:text-lg font-bold font-['Rajdhani',sans-serif] text-slate-900 dark:text-white group-hover:text-cyan-400 transition-colors leading-tight">
            {p.title}
          </h3>
          ...
          {/* Key Measured Impact */}
          {p.impact && (
            <div className="mt-2 flex items-start gap-1 p-1.5 rounded-lg bg-cyan-500/10 border border-cyan-500/25 text-cyan-600 dark:text-cyan-300 font-mono text-[10.5px]">
              <Zap size={12} className="text-cyan-400 shrink-0 mt-0.5 animate-pulse" />
              <span className="font-semibold leading-tight line-clamp-1">{p.impact}</span>
            </div>
          )}
          ...
          {p.tech && (
            <div className="mt-2 flex flex-wrap gap-1">
              {p.tech.slice(0, 3).map((t, idx) => (
                <span
                  key={idx}
                  className="text-[8.5px] font-mono px-1.5 py-0.2 rounded bg-slate-100 dark:bg-[#151822] text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-white/5 font-medium"
                >
                  {t}
                </span>
              ))}
              {p.tech.length > 3 && (
                <span className="text-[8.5px] font-mono px-1 py-0.2 rounded text-cyan-400 font-bold">
                  +{p.tech.length - 3}
                </span>
              )}
            </div>
        ...
        {/* Actions & Links */}
        <div className="mt-3 pt-2 border-t border-slate-100 dark:border-white/5 flex items-center justify-between gap-1.5">
          <div className="flex items-center gap-1.5">
            {p.live && p.live !== "Coming Soon" ? (
              <a
                ...
                className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-[11px] font-mono font-bold hover:scale-105 transition shadow-[0_0_10px_rgba(6,182,212,0.3)]"
              >
                <FaExternalLinkAlt size={9} />
                <span>Live Demo</span>
              </a>
            ) : (
              <span className="text-[10px] font-mono text-slate-400 dark:text-slate-500 px-2 py-0.5 bg-slate-100 dark:bg-[#151822] rounded">
                Coming Soon
              </span>
            )}
            ...
            {p.github && (
              <a
                ...
                onMouseEnter={() => soundFX.playHover()}
                className="flex items-center gap-1 px-2 py-1 rounded-lg border border-slate-300 dark:border-white/10 hover:border-cyan-400 text-slate-700 dark:text-cyan-300 text-[11px] font-mono transition"
          ...
          <button
            ...
            onMouseEnter={() => soundFX.playHover()}
            className="flex items-center gap-1 px-2 py-1 rounded-lg text-slate-500 dark:text-slate-400 hover:text-cyan-500 dark:hover:text-cyan-400 hover:bg-cyan-500/10 text-[11px] font-mono font-bold transition"
  ...
  return (
    <section
      id="projects"
      className="relative flex flex-col justify-center px-1 sm:px-4 py-2 sm:py-3 overflow-hidden text-slate-900 dark:text-white"
    >
      {/* Ambient Glows */}
      ...
      <div className="max-w-7xl mx-auto w-full relative">
        ...
        {/* SECTION HEADER */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-3 mb-2.5 sm:mb-3">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-slate-900 dark:text-[#f8fafc] font-['Rajdhani',sans-serif] tracking-tight leading-tight">
              Artifacts &{" "}
          ...
          <div className="flex flex-wrap items-center gap-2 self-start md:self-auto">
            {/* View Mode Switcher */}
            <div className="flex items-center p-0.5 rounded-lg bg-white/80 dark:bg-[#12141c] border border-slate-200 dark:border-white/10 shadow-sm">
              <button
                onClick={() => {
                  soundFX.playClick();
                  setViewMode("carousel");
                }}
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-mono font-bold transition ${
                  viewMode === "carousel"
            ...
            {viewMode === "carousel" && (
              <div className="flex items-center gap-1">
                <button
                  ...
                  onMouseEnter={() => soundFX.playHover()}
                  className="p-2 rounded-lg border border-slate-300 dark:border-white/10 bg-white dark:bg-[#101218] text-slate-800 dark:text-cyan-300 hover:border-cyan-400 transition active:scale-95"
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 mb-2.5">
            ...
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  onMouseEnter={() => soundFX.playHover()}
                  className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-mono font-bold whitespace-nowrap transition-all duration-200 ${
                    isSelected
                      ? "bg-gradient-to-r from-cyan-500 to-violet-600 text-white shadow-[0_0_10px_rgba(6,182,212,0.35)]"
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <div className="hidden xs:inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-100 dark:bg-[#141722] border border-slate-200 dark:border-white/10 text-[9.5px] font-mono text-cyan-600 dark:text-cyan-400 font-bold shrink-0">
              <span>{filtered.length} of {projects.length}</span>
            </div>

            <div className="relative flex-1 sm:min-w-[190px] sm:max-w-xs">
              <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search stack, problems..."
                className="w-full pl-8 pr-7 py-1 rounded-lg bg-white/80 dark:bg-[#12141c] border border-slate-200 dark:border-white/10 text-[11px] font-mono text-slate-800 dark:text-slate-200 placeholder-slate-400 focus:outline-none focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400 transition"
              />
              {searchQuery && (
                <button
        ...
        {filtered.length === 0 ? (
          <div className="py-12 text-center rounded-2xl border border-slate-200 dark:border-white/10 bg-white/60 dark:bg-[#101218]/80 backdrop-blur-xl">
            <p className="font-mono text-slate-400 text-xs mb-2.5">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 font-mono text-xs font-bold hover:bg-cyan-500/20 transition"
            >
              <RotateCcw size={11} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5 pb-4 pt-1 max-h-[58vh] overflow-y-auto no-scrollbar">
            {filtered.map((p, i) => (
              <motion.div
                key={p.title}
          ...
          <>
            <div
              ...
              className="flex gap-3.5 overflow-x-auto pb-2.5 pt-1 snap-x snap-mandatory scroll-smooth cursor-grab active:cursor-grabbing no-scrollbar"
            >
              {filtered.map((p, i) => (
                <motion.div
                  key={p.title}
                  ...
                  className="snap-start shrink-0 w-[270px] sm:w-[320px] md:w-[340px] group"
                >
                  {renderProjectCard(p, i)}
                </motion.div>
              ))}
            ...
            <div className="mt-1 flex items-center gap-2.5">
              <div className="flex-1 h-1 bg-slate-200 dark:bg-[#12141c] rounded-full overflow-hidden border border-slate-300 dark:border-white/5">
                <div
                  className="h-full bg-gradient-to-r from-cyan-500 via-blue-500 to-violet-500 rounded-full transition-all duration-150 shadow-[0_0_8px_#00f0ff]"
                  style={{ width: `${Math.max(scrollProgress, 10)}%` }}
                />
              </div>
              <span className="text-[10px] font-mono text-cyan-400 shrink-0 font-bold">
                {filtered.length} of {projects.length} Raids
              </span>
            </div>
          </>
        )}
